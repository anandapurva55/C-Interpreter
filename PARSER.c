...
{SYNTAX, UNBAL_PARENS, NO_EXP , EQUALS_EXPECTED , NOT_VAR , PARAM_ERR , SEMI_EXPECTED ,
    ...
    NEST_FUNC, RET_NOCALL, PAREN_EXPECTED, WHILE_EXPECTED,
    QUOTE_EXPECTED, NOT_TEMP, TOO_MANY_LVARS, DIV_BY_ZERO};
...
/* Standard library" functions are declared here so they can be put into the internal function table that follows. */
...
void sntx_err(int error){
    ...
    printf("\n%s", e[error]);
    p = p_buf;
    while(p != p_buf){ // find the line number of the error
        p++;
        if(*p == '\r'){
            linecount++;
        }
...
int get_token(void){
    ...
    *temp = '\0';
    ...
    if(*prog == '\0') {/*end of file */
        *token = '\0';
        tok = FINISHED;
        return (token_type = DELIMETER) ; //delimeter includes puntuation and operators
        
    }
    
    if(strchr("{}" , *prog)){  /*  block delimters  */
        *temp = *prog;
        temp++;
        *temp = '\0';
        prog++;
    ...
    *temp = '\0';
    ...
    if(token_type == TEMP)  {
        ...
        if(tok) token_type = KEYWORD; /* it is a keyword */
        else token_type = IDENTIFIER;
    }
    return token_type;
}

/* Return a token to input strem. */
void putback(void){
    char *t;
