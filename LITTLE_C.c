...
//  Interpreter
//
//  Created by apurva anand on 9/30/18.
...
char *prog;  // current location in the source code.
...
/* interp_block() Function decides what action to take based upon the next token in the input stream
 Interpret a single statement or block of code.When interp_block() returns from its initial call,
 the final brace (or a retuen) in main has been encountered
 */
        ...
        if(token_type == IDENTIFIER) { /* Not a keyword, so process expression. */
            ...
            switch(tok) {
                ...
                case RETURN: /* return from function call */
                    func_ret();
                    return;
                case IF: exec_if();  /* process an if statement */
                    break;
...
void decl_local(void){
    ...
    do{ // procee comma separated list
        get_token();
...
void call(void){
    ...
    if(loc == NULL)
        ...
        get_args(); //get fun ction argunments
        temp = prog;
        func_push(lvartemp);
        prog = loc; //reset prog to start of function
        get_params(); //load the function's parameters with the value of the arguments
...
/* Push the arguments tpo a function onto the local variable stack */
void get_args(void){
    int value, count;
    int temp[NUM_PARAMS];
    struct var_type i;
    ...
    //process a comma separested list of values
    do{
        eval_exp(&value);
...
void get_params(void){
    ...
    do { //process comma-separated list of parameters
        ...
        if(*token != ')') {
            ...
            // link parameters name with arguments already on local var stack
...
int find_var(char *s){
    ...
    //else see if it's global variable
...
//Determine if the identifier is a variable.
//Return 1 if it's variable else 0
...
void exec_if(void){
    ...
    if(cond) {//is true so process target pf IF
        interp_block();
    }
    else {// else skip IF and process ELSE, if present
...
void exec_do(void){
    ...
    putback(); //save location of top of do loop
    ...
    if(cond) prog = temp; // if true loop;else continue on
}
...
void exec_for(void){
    ...
    for(; ;){
        ...
        else { // else skip around loop
            find_eob();
            return;
        }
        prog = temp2;
        eval_exp(&cond); //increment
        prog = temp; // loop back to top
    }
}
